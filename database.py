...
def deactivate_product(product_id):
    """Mahsulotni ro'yxatdan yashiradi (o'chirmaydi, tarixiy buyurtmalar buzilmasin uchun)."""
    conn = get_conn()
...
def find_order_by_daily_number(user_id, daily_number):
    """Mijozning o'zi bergan buyurtmasini navbat raqami bo'yicha topadi."""
    conn = get_conn()
    row = conn.execute(
        "SELECT * FROM orders WHERE user_id = ? AND daily_number = ? "
        "ORDER BY id DESC LIMIT 1",
        (user_id, daily_number),
    ).fetchone()
    conn.close()
    return dict(row) if row else None
